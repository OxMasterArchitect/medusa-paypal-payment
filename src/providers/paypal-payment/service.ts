...
> {
  ...
  constructor(
    ...
    if (this.options_.oAuthClientId && this.options_.oAuthClientSecret && this.options_.environment) {
      this.paypal_ = new PaypalClient({
        ...
        timeout: 0,
        environment: this.options_.environment === "sandbox" ? Environment.Sandbox : Environment.Production,
        logging: {
   ...
   try {
      ...
      return {
        data: {
          ...
          paypalOrderId: (retrievedPayment.data!.paypalOrder! as PaypalOrder).id
        },
        status: status
      }
    } catch (error) {
      ...
      const { result, ...httpResponse } = await ordersController.createOrder({
        body: {
          ...
          purchaseUnits: [
            {
              amount: {
                currencyCode: paymentData.currency_code,
  ...
  async getPaymentStatus(paymentData: GetPaymentStatusInput): Promise<GetPaymentStatusOutput> {
    ...
    const retrievedPayment = await this.retrievePayment(
      paymentData
    );
  ...
  async retrievePayment(paymentSessionData: RetrievePaymentInput): Promise<RetrievePaymentOutput> {
    const ordersController = new OrdersController(this.paypal_);
    try {
      const paypalOrderId = (paymentSessionData.data!.paypalOrder as PaypalOrder).id as string
  ...
  async updatePayment(context: UpdatePaymentInput): Promise<UpdatePaymentOutput> {
    const ordersController = new OrdersController(this.paypal_);
    try {
      const paypalOrderId = (context.data!.paypalOrder as PaypalOrder).id as string
